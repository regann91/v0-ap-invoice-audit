...
// ── Collapsible Section (replaces antd Collapse to avoid Panel warning) ──
  ...
  return (
    ...
    >
      <div style={{ maxHeight: 520, overflowY: "auto", paddingRight: 4 }}>
        ...
        <pre style={{ background: "#f6f8fa", border: "1px solid #e8e8e8", borderRadius: 4, padding: "10px 12px", fontSize: 12, lineHeight: 1.6, whiteSpace: "pre-wrap", fontFamily: "monospace", marginBottom: 12 }}>
          {d.systemPrompt}
        </pre>
        <Text type="secondary" style={{ display: "block", marginBottom: 8, fontSize: 13 }}>User Prompt Template</Text>
        <pre style={{ background: "#f6f8fa", border: "1px solid #e8e8e8", borderRadius: 4, padding: "10px 12px", fontSize: 12, lineHeight: 1.6, whiteSpace: "pre-wrap", fontFamily: "monospace" }}>
          ...
          <CollapsibleSection title={<Text strong style={{ fontSize: 13 }}>Platform Input Config</Text>}>
            ...
            <div className="flex gap-4 mt-3">
              <div style={{ flex: 1 }}>
                ...
                <InputNumber defaultValue={d.temperature} min={0} max={1} step={0.1} style={{ width: "100%" }} disabled={readonlyInput} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={labelStyle}>Max Tokens</label>
                <InputNumber defaultValue={d.maxTokens} min={1} step={256} style={{ width: "100%" }} disabled={readonlyInput} />
              </div>
            </div>
              ...
              <div style={{ border: "1px solid #f0f0f0", borderRadius: 4, overflow: "hidden" }}>
                    ...
                    {params.map((p, i) => (
                      <tr key={i}>
                        <td style={{ padding: "6px 10px", borderBottom: "1px solid #f8f8f8" }}>
                          <Input value={p.key} size="small" disabled={readonlyInput} onChange={(e) => updateParam(i, "key", e.target.value)} style={{ fontFamily: "monospace", fontSize: 12 }} />
                        </td>
                        <td style={{ padding: "6px 10px", borderBottom: "1px solid #f8f8f8" }}>
                          <Input value={p.value} size="small" disabled={readonlyInput} onChange={(e) => updateParam(i, "value", e.target.value)} style={{ fontFamily: "monospace", fontSize: 12 }} />
                        </td>
                        {isOps && (
                ...
                {isOps && (
                  <div style={{ padding: "8px 10px" }}>
                    <Button type="dashed" icon={<PlusOutlined />} size="small" onClick={addParam} style={{ fontSize: 12 }}>Add Param</Button>
                  </div>
                )}
              </div>
            </div>
