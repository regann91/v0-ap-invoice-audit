...
export function FeedbackSuggestionList({ onViewRunDetail }: FeedbackSuggestionListProps) {
  ...
  const filtered = useMemo(() => {
    return data.filter((r) => {
      ...
      const matchSearch = !q || r.runId.toLowerCase().includes(q) || r.agentName.toLowerCase().includes(q)
      ...
      return matchSearch && matchStep && matchStatus && matchDate
    })
  }, [data, search, dateRange, stepFilter, statusFilter])
  ...
  const hasFilters = !!(search || dateRange || stepFilter || statusFilter)
